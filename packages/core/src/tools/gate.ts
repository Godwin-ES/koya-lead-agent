...
export function gate(run: GateRunState, toolName: string, input: Record<string, unknown>): GateDecision {
  ...
  // finalize_run is exempt from these two checks for the same reason it's
  // exempt from the ICP check above: it's the runner's own turn-limit
  // safety valve (worker/src/runners/gemini.ts / agent-sdk.ts), called
  // directly rather than by the model, and it must be able to close a run
  // out gracefully even if the tool-call or spend budget was *also*
  // already exhausted - otherwise that safety-valve call gets denied too,
